...
def rnd_conf():
    return round(random.uniform(0.72, 0.99), 3)
...
def rnd_bbox():
    return (
        ...
        random.randint(320, 700),
        random.randint(220, 550),
    )
...
async def seed_zones(sess):
    ...
    rows = [
        ...
        (
            ...
            "cam-003",
            "site-hq",
            '["no hardhat","no vest","no goggles","no gloves"]',
        ),
        (
            "zone-chemical",
            "Chemical Store",
            ...
            "cam-004",
            "site-hq",
            '["no hardhat","no vest","no mask","no gloves"]',
        ),
        ...
        (
            ...
            "cam-006",
            "site-power",
            '["no hardhat","no vest","no boots","no goggles"]',
        ),
        (
            "zone-crane",
            "Crane Area",
        ...
        (
            ...
            "cam-009",
            "site-pharma",
            '["no mask","no gloves","no goggles"]',
        ),
        ("zone-control", "Control Room", "general", "cam-010", "site-power", '["no hardhat"]'),
    ]
    for zid, zname, ztype, cam, _site, ppe in rows:
        await sess.execute(
            text("""
...
async def seed_workers(sess):
    ...
    for wid, name, dept, shift, role, _site, risk, rlevel in rows:
        await sess.execute(
            text("""
    ...
    for day in range(90, 0, -1):
        ...
        for _ in range(n):
            ...
            """),
                dict(
                    tid=track,
                    cls=random.choice(VIOLATION_CLASSES),
                    conf=rnd_conf(),
                    zone=zones[zi],
                    cam=cams[zi % len(cams)],
                    site="site-hq",
                    x1=x1,
                    y1=y1,
                    x2=x2,
...
async def seed_proximity_alerts(sess):
    ...
    for i in range(12):
        ...
        """),
            dict(
                ptid=random.randint(200, 300),
                mtid=random.randint(400, 500),
    ...
    for i in range(15):
        ...
        """),
            dict(
                ...
                rca="Root cause: Inadequate PPE enforcement at zone entry. Worker bypassed checkpoint without proper gear inspection.",
                actions="1. Zone entry blocked until PPE complied.\n2. Supervisor notified within 2 minutes.\n3. Worker issued fresh PPE and re-briefed.",
                ...
                osha=random.choice(
                    ["OSHA 1926.100", "OSHA 1910.132", "IS 4770", "IS 15748", "IS 818"]
                ),
                sev_level=sev,
                sev=sev,
...
async def seed_webhooks(sess):
    ...
    rows = [
        (
            ...
            "general",
            json.dumps(["violation.created", "fire.detected"]),
            1,
        ),
        (
            "HRMS Integration",
            "https://hrms.example.com/webhooks/safety",
            "general",
            json.dumps(["worker.violation", "permit.approved"]),
            1,
        ),
        ("ERP Webhook", "https://erp.example.com/api/events", "general", json.dumps(["all"]), 0),
    ]
    for name, url, wtype, events, active in rows:
        ...
        """),
            dict(
                ...
                wtype=wtype,
                events=events,
                secret=f"whsec_{hashlib.sha256(name.encode()).hexdigest()[:16]}",
                active=active,
            ),
        )
...
async def seed_audit_log(sess):
    ...
    rows = [
        (
            ...
            "permit",
            "PTW-20260601-ABC123",
            "W004 hot_work permit approved by sup-001",
        ),
        (
            "worker.checkin",
            "system",
            "worker",
            "W001",
            "W001 Ravi Kumar checked in via face recognition at site-hq",
        ),
        (
            "violation.ack",
            "safety_off",
            "violation",
            "550",
            "no hardhat in zone-welding acknowledged",
        ),
        (
            "org.created",
            "admin",
            "org",
            "org-pharma-hyd",
            "New org org-pharma-hyd created with starter plan",
        ),
        (
            "billing.subscribe",
            "admin",
            "billing",
            "org-steel-india",
            "org-steel-india subscribed to growth plan INR 14999/mo",
        ),
        (
            "alert.escalated",
            "system",
            "alert",
            "esc-007",
            "L2 escalation triggered for W007 no mask chemical zone",
        ),
        (
            "worker.checkout",
            "system",
            "worker",
            "W002",
            "W002 Suresh Raj checkout after 8.5 hours site-hq",
        ),
        (
            "report.generated",
            "system",
            "report",
            "RPT-20260608-0001",
            "Incident report generated for no hardhat violation",
        ),
        (
            "zone.alert",
            "system",
            "zone",
            "zone-chemical",
            "Unauthorized entry detected zone-chemical cam-004",
        ),
        (
            "permit.closed",
            "W004",
            "permit",
            "PTW-20260601-DEF456",
            "Electrical work completed safely permit closed",
        ),
        (
            "camera.restart",
            "operator",
            "camera",
            "cam-009",
            "cam-009 QC Lab restarted due to stream timeout",
        ),
        (
            "worker.enroll",
            "admin",
            "worker",
            "W015",
            "W015 Karthi Vel face enrolled for recognition",
        ),
    ]
...
async def seed_model_deployments(sess):
    ...
    rows = [
        ...
        (
            ...
            "active",
            2,
            "Promoted after canary passed — +3.2% mAP",
        ),
        (
            "ppe-detector",
            "v2.3.0",
        ...
        (
            ...
            "rolled_back",
            9,
            "Rolled back: latency regression +18%",
        ),
    ]
    count = 0
...
async def seed_industry_ppe_profiles(sess):
    ...
    rows = [
        ...
        (
            "construction",
            "hazardous",
            '["no hardhat","no vest","no gloves","no boots"]',
            "CRITICAL",
            "IS 2925 / IS 4770",
            "All PPE mandatory in hazardous zones per Factories Act 1948",
        ),
        (
            "construction",
            "restricted",
            '["no hardhat","no vest","no boots"]',
            "HIGH",
            "IS 2925",
            "Head/foot/visibility PPE required in restricted zones",
        ),
        (
            "construction",
            "general",
            ...
            "MEDIUM",
            "IS 4770",
            "Minimum PPE for general construction zones",
        ),
        (
            "steel_manufacturing",
            "hazardous",
            '["no hardhat","no gloves","no goggles","no boots","no vest"]',
            "CRITICAL",
            "IS 5983 / IS 6994",
            "Full PPE for hot-work and molten-metal areas",
        ),
        (
            "steel_manufacturing",
            "restricted",
            ...
            "HIGH",
            "IS 2925 / IS 6994",
            "Heat-resistant gloves mandatory in steel handling zones",
        ),
        (
            "steel_manufacturing",
            "general",
            ...
            "MEDIUM",
            "IS 4770",
            "Hi-vis and head protection throughout facility",
        ),
        (
            "oil_gas",
            "hazardous",
            '["no hardhat","no mask","no gloves","no boots","no vest"]',
            "CRITICAL",
            "OISD-STD-173",
            "Full PPE + ATEX-rated equipment in hazardous areas",
        ),
        (
            "oil_gas",
            "restricted",
            '["no hardhat","no mask","no gloves"]',
            "HIGH",
            "OISD-STD-173",
            "Respiratory protection mandatory in gas-risk zones",
        ),
        (
            "oil_gas",
            "general",
            ...
            "MEDIUM",
            "OISD-STD-173",
            "Minimum PPE across all oil & gas site areas",
        ),
        (
            "pharma",
            "hazardous",
            '["no mask","no gloves","no goggles","no vest"]',
            "CRITICAL",
            "GMP Schedule M",
            "Clean-room and chemical PPE per Drug & Cosmetics Rules",
        ),
        (
            "pharma",
            "restricted",
            ...
            "HIGH",
            "GMP Schedule M",
            "Lab PPE for API handling and formulation areas",
        ),
        (
            "pharma",
            "general",
            ...
            "MEDIUM",
            "IS 9473",
            "Face mask mandatory in all pharma production areas",
        ),
        (
            "warehouse",
            "hazardous",
            ...
            "HIGH",
            "IS 4770",
            "PPE for forklift and heavy-goods areas",
        ),
        (
            "warehouse",
            "general",
            ...
            "LOW",
            "IS 4770",
            "Hi-vis vest required throughout warehouse floor",
        ),
        (
            "power_plant",
            "hazardous",
            '["no hardhat","no goggles","no boots","no gloves","no vest"]',
            "CRITICAL",
            "CEA Safety Regs",
            "Full electrical PPE for live-panel and boiler access",
        ),
    ]
    ...
    for i in range(8):
        ...
        """),
            dict(
                ...
                details=json.dumps(
                    {"feature": "confidence_dist", "samples": random.randint(500, 2000)}
                ),
                ts=ts.strftime("%Y-%m-%d %H:%M:%S"),
            ),
        )
    ...
    for _i in range(14):
        ...
        """),
            dict(
                ...
                ackat=(ts + timedelta(minutes=random.randint(3, 30))).strftime("%Y-%m-%d %H:%M:%S")
                if ack
                else None,
                ts=ts.strftime("%Y-%m-%d %H:%M:%S"),
            ),
...
async def seed_permits(sess):
    ...
    for wid, sup, wtype, status, day_offset in rows:
        ...
        vf = (
            ago(hours=random.randint(1, 8))
            if status in ("active", "closed")
            else now() + timedelta(hours=day_offset * 8)
        )
