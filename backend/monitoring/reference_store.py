...
REFERENCE_PATH = pathlib.Path(
    os.getenv("REFERENCE_STATS_PATH", "./monitoring/reference_stats.json")
)
...
ALLOWED_REFERENCE_DIRS = [
    ...
    for d in os.getenv("ALLOWED_REFERENCE_DIRS", "./monitoring").split(",")
    if d.strip()
]
...
class ReferenceStats(BaseModel):
    ...
    conf_values: list[float] = Field(
        ..., min_length=100
    )  # FIXED: min_items → min_length (Pydantic v2)
...
def load_reference(
    ...
    if not ref_path.exists():
        logger.warning(
            "Reference stats not found at {} — " "run monitoring.reference_store after training",
            _redact_path(str(ref_path)),
        )
        return None
...
def validate_reference(reference: dict[str, Any] | None) -> list[str]:
    """
...
def get_reference_diagnostics(config: ReferenceConfig | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "validation": validate_reference(load_reference(cfg))
        if ref_path.exists()
        else ["Not loaded"],
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--summary", default="models/training_summary.json", help="Path to training summary JSON"
    )
    parser.add_argument(
        "--output", default=str(REFERENCE_PATH), help="Output path for reference stats"
    )
    parser.add_argument(
        "--samples", type=int, default=5000, help="Number of confidence samples to store"
    )
    ...
    print("Building reference from training summary:")
