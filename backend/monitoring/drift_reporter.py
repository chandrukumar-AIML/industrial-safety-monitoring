...
ALLOWED_REPORT_DIRS = [
    pathlib.Path(d).resolve() for d in os.getenv("ALLOWED_REPORT_DIRS", "./monitoring").split(",")
]
...
if SLACK_WEBHOOK and not SLACK_WEBHOOK.startswith("https://"):
    logger.warning(
        "SLACK_DRIFT_WEBHOOK does not start with https:// — disabling to prevent insecure send"
    )
    SLACK_WEBHOOK = ""
...
async def get_todays_stats(
    db_factory: DBFactoryProtocol, days_back: int = 1
) -> dict[str, Any] | None:
...
async def save_drift_result(
    result: DriftResult,  # type: ignore[name-defined]  # noqa: F821
    check_date: date,
    report_path: str,
...
async def run_daily_drift_check(
    db_factory: DBFactoryProtocol, config: ReportConfig | None = None
) -> DriftResult | None:  # type: ignore[name-defined]  # noqa: F821
    """
    ...
    report_path = generate_html_report(result, prod_stats, reference, date.today(), cfg)
