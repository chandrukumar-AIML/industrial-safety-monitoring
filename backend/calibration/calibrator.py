...
CRITICAL_M = _validate_positive_float(
    "PROXIMITY_CRITICAL_M", os.getenv("PROXIMITY_CRITICAL_M", "2.0"), 2.0
)
WARNING_M = _validate_positive_float(
    "PROXIMITY_WARNING_M", os.getenv("PROXIMITY_WARNING_M", "5.0"), 1.0
)
...
class CalibrationData(BaseModel):
    ...
    def validate_consistency(self) -> CalibrationData:
        ...
        if abs(det) < 1e-6:
            logger.warning(
                "Homography matrix is near-singular (det={}) — calibration may be unstable", det
            )
...
def _get_calibration_path(camera_id: str, filename: str | None = None) -> pathlib.Path:
    """
...
class CameraCalibration:
    ...
    def __init__(
        ...
        if abs(det) < 1e-6:
            logger.warning(
                "Homography matrix is near-singular (det={:.2e}) — distance calculations may be inaccurate",
                det,
            )
    ...
    def from_point_pairs(
        ...
        if len(pixel_points) != len(real_world_points):
            raise ValueError(
                f"Mismatched point counts: {len(pixel_points)} pixel vs {len(real_world_points)} real"
            )
        ...
        for i, (rx, ry) in enumerate(real_world_points):
            if not (-1000 <= rx <= 1000 and -1000 <= ry <= 1000):  # Reasonable real-world bounds
                logger.warning(
                    "Real-world point {} seems large: ({}, {}) — verify measurement", i, rx, ry
                )
    ...
    def pixel_to_real(self, px: float, py: float) -> tuple[float, float]:
        ...
        if not (0 <= px <= 10000 and 0 <= py <= 10000):
            logger.warning(
                "Pixel coords out of expected range: ({}, {}) — result may be inaccurate", px, py
            )
    ...
    def load(
        ...
        try:
            ...
            if not validated.validated:
                logger.warning(
                    "Loaded calibration is not validated — distance calculations may be inaccurate"
                )
    ...
    def verify(
        self, test_points: list[tuple[tuple[float, float], tuple[float, float]]] | None = None
    ) -> dict[str, Any]:
        """
