...
if TYPE_CHECKING:
    ...
    from .inference.pipeline import FrameResult, InferencePipeline
...
class AppState:
    ...
    latest_frame: FrameResult | None = None
    ...
    )  # threading.RLock, set in __post_init__
    ...
    def set_latest_frame(self, frame: FrameResult | None) -> None:
        """Thread-safe update of latest_frame from the pipeline worker."""
        with self._lock:
            self.latest_frame = frame

    def get_latest_frame(self) -> FrameResult | None:
    ...
    async def get_latest_frame_async(self) -> FrameResult | None:
        """Backward-compatible async wrapper for route/test helpers."""
        return self.get_latest_frame()

    def set_pipeline(self, pipeline: InferencePipeline | None) -> None:
        with self._lock:
            self.pipeline = pipeline

    def get_pipeline(self) -> InferencePipeline | None:
        with self._lock:
            return self.pipeline

    def set_pipeline_runtime(self, runtime: PipelineRuntime | None) -> None:
        with self._lock:
            self.pipeline_runtime = runtime

    def get_pipeline_runtime(self) -> PipelineRuntime | None:
        with self._lock:
            return self.pipeline_runtime

    def set_shap_explainer(self, explainer: SHAPExplainer | None) -> None:
        with self._lock:
            self.shap_explainer = explainer

    def get_shap_explainer(self) -> SHAPExplainer | None:
        with self._lock:
            return self.shap_explainer

    def set_pipeline_status(self, status: str, error: str | None = None) -> None:
        with self._lock:
            self.pipeline_status = status
            self.pipeline_error = error

    def get_pipeline_status(self) -> tuple[str, str | None]:
        with self._lock:
