...
class WorkerProfile(BaseModel):
    ...
    department: str | None = Field(default=None, max_length=MAX_DEPARTMENT_LEN)
    role: str | None = Field(default=None, max_length=100)

    # Identity fields
    face_embedding: bytes | None = Field(default=None, exclude=True)  # Serialized embedding
    ...
    def to_api_response(self, include_pii: bool = False) -> dict:
        ...
        if PRIVACY_MODE == "strict" and not include_pii:
            # Redact PII
            data["full_name"] = (
                f"Worker***{self.worker_id[-4:]}" if len(self.worker_id) >= 4 else "Worker***"
            )
            data["email"] = None
            if self.department:
    ...
    async with db_factory() as session:
        try:
            ...
            stmt = insert(DBModel).values(
                ...
                enrolled_at=validated.enrolled_at or datetime.now(UTC),
...
async def anonymize_worker(
    ...
    async with db_factory() as session:
        ...
        result = await session.execute(
            text("SELECT 1 FROM worker_profiles WHERE worker_id = :id"), {"id": worker_id_safe}
        )
        if not result.first():
            return False
        ...
        await session.execute(
            update(DBModel).where(DBModel.worker_id == worker_id_safe).values(**update_values)
        )
    ...
    logger.warning(
        ...
        _redact_worker_id(worker_id_safe),
        reason[:100],
    )
    return True
    ...
    async with db_factory() as session:
        try:
            if hard_delete:
                ...
                return await anonymize_worker(
                    worker_id_safe, db_factory, reason="Soft delete request"
                )
...
async def get_active_workers(
    ...
    async with db_factory() as session:
        query = select(DBModel).where(DBModel.status != WorkerStatus.ANONYMIZED.value)
        if status_filter:
    ...
    async with db_factory() as session:
        ...
        enrolled = await session.execute(
            select(func.count()).where(
                DBModel.face_embedding is not None,
                DBModel.status != WorkerStatus.ANONYMIZED.value,
            )
        )
