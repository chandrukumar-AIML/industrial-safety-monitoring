...
def _validate_odd_int(
    name: str, value: str, default: int, min_val: int = 15, max_val: int = 99
) -> int:
    try:
        val = int(value)
...
_DEFAULT_BLUR_KERNEL = _validate_odd_int(
    "FACE_BLUR_KERNEL", os.getenv("FACE_BLUR_KERNEL", "51"), 51
)
...
class FaceBlurrer:
        ...
        if self._detector_backend in ("dnn", "auto"):
            try:
                ...
                if self._detector_backend == "dnn":
                    logger.error(
                        "DNN detector requested but failed: {} — falling back to Haar", exc
                    )
    ...
    def blur(
        ...
        for _i, (x, y, w, h) in enumerate(faces):
    ...
    def save_blurred(
        ...
        allowed_dirs = [
            Path(d).resolve()
            for d in os.getenv("ALLOWED_OUTPUT_DIRS", "./storage/blurred").split(",")
        ]
        ...
        success = cv2.imwrite(str(out_path), blurred, [cv2.IMWRITE_JPEG_QUALITY, quality])
...
_face_blurrer_instance: FaceBlurrer | None = None
