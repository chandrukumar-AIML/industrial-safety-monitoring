...
class FaceRecognizer:
    ...
    def __init__(
        ...
        rate_limiter: RecognitionRateLimiter | None = None,
    ) -> None:
        ...
        for row in rows:
            try:
                ...
                if embedding.ndim != 1 or not 128 <= len(embedding) <= 2048:
                    logger.warning(
                        "Invalid embedding shape for {}: {} — skipping", worker_id, embedding.shape
                    )
                    continue
        ...
        try:
            ...
            for rep in representations[:max_faces]:
                ...
                if best_match:
                    ...
                    if use_cache:
                        frame_hash = (
                            f"{frame_bgr.shape}:{frame_bgr[::50, ::50].tobytes()[:1000].hex()}"
                        )
    ...
    def _clean_recognition_cache(self, max_age_s: float = 5.0) -> None:
        ...
        now = time.time()
        stale_keys = [k for k, (_, ts) in self._recognition_cache.items() if now - ts > max_age_s]
        for k in stale_keys:
            del self._recognition_cache[k]

    def invalidate_cache(self, worker_id: str | None = None) -> None:
            ...
            workers.append(
                {
                    "worker_id": wid,
                    "name": display_name,
                    "enrolled_at": datetime.fromtimestamp(enrolled_ts, tz=UTC).isoformat(),
                }
            )
        return workers
...
_face_recognizer_instance: FaceRecognizer | None = None
