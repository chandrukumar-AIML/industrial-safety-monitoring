...
EVAL_INTERVAL_S = _validate_float_range(
    "MLOPS_EVAL_INTERVAL_S", os.getenv("MLOPS_EVAL_INTERVAL_S", "60.0"), 60.0, 10.0, 3600.0
)
...
async def start_canary_deployment(
    ...
    router: CanaryRouterProtocol | None = None,
) -> int | None:
    """
    ...
    from .model_registry import MODEL_NAME, get_production_model, get_staging_models
        ...
        try:
            result = await session.execute(
                ...
                """),
                {
                    ...
                    "pct": cfg.canary_pct
                    if hasattr(cfg, "canary_pct")
                    else float(os.getenv("CANARY_TRAFFIC_PCT", "10")),
                    "notes": notes,
                },
            )
    ...
    router_to_use = (
        router
        or __import__("backend.mlops.canary_router", fromlist=["canary_router"]).canary_router
    )
    try:
        ...
        async with db_factory() as session:
            await session.execute(
                text("DELETE FROM model_deployments WHERE id=:id"), {"id": deployment_id}
            )
            await session.commit()
...
async def promote_canary(
    ...
    if success:
        ...
        # Notify Slack
        await _notify_slack(
            f"✅ Canary PROMOTED to Production | " f"v{version} now serves 100% traffic | {reason}",
            webhook=cfg.slack_webhook,
        )
    ...
    while True:
        try:
            ...
            router_to_use = (
                router
                or __import__(
                    "backend.mlops.canary_router", fromlist=["canary_router"]
                ).canary_router
            )
            ...
            # Evaluate canary
            from .canary_evaluator import EvaluationVerdict, evaluate_canary
...
async def _notify_slack(message: str, webhook: str | None = None) -> None:
    """
