...
def _parse_zone(zone_def: dict) -> tuple[str, np.ndarray] | None:
    ...
    try:
        ...
        logger.warning(
            "Zone '{}' polygon could not be converted to array: {} — skipping", zone_id, exc
        )
        return None

    if polygon.ndim != 2:
        logger.warning(
            "Zone '{}' polygon must be 2-D (N×2), got ndim={} — skipping", zone_id, polygon.ndim
        )
        return None
    if polygon.shape[1] != _VERTEX_DIMS:
        logger.warning(
            "Zone '{}' each vertex must have {} coordinates, got {} — skipping",
            zone_id,
            _VERTEX_DIMS,
            polygon.shape[1],
        )
    ...
    if len(polygon) < _MIN_VERTICES:
        logger.warning(
            "Zone '{}' polygon must have >= {} vertices, got {} — skipping",
            zone_id,
            _MIN_VERTICES,
            len(polygon),
        )
...
def load_zones(
    ...
    for _i, zone_def in enumerate(cfg.get("zones", [])):
