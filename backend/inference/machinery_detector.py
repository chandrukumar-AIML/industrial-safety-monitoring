...
MACHINERY_TRACK_BUFFER = int(
    os.getenv("MACHINERY_TRACK_BUFFER", "60")
)  # Longer buffer for slow-moving machinery
...
class MachineryDetector:
    ...
    def __init__(
        self,
        model_path: str | Path = MACHINERY_MODEL_PATH,
        device: str = "cpu",
    ...
    def detect(
        ...
        machinery_mask = np.array(
            [self.class_names[cid].lower() in MACHINERY_CLASSES for cid in class_ids]
        )
        ...
        for i in range(len(tracked)):
            ...
            name = self.class_names[cid] if cid < len(self.class_names) else f"machine_{cid}"
            output.append(
                MachineryDetection(
...
_machinery_detector_instance: MachineryDetector | None = None
