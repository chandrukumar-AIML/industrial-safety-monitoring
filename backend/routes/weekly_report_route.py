...
_ALLOWED_REPORT_DIRS = [
    ...
    for d in ["./reports", os.getenv("WEEKLY_REPORT_OUTPUT_DIR", "./reports/weekly")]
]
    ...
    if reference_date:
        try:
            ...
            raise HTTPException(
                ...
                "reference_date must be ISO format (YYYY-MM-DD)",
            )
    asyncio.create_task(
        generate_and_send(AsyncSessionLocal, ref, send_email), name="weekly_report_manual"
    )

    return {
        "status": "generating",
        "message": "Report generation started. Check /weekly-reports in 30s.",
    }


@router.get(
    "/{report_id}/download", response_class=FileResponse, summary="Download weekly report PDF"
)
