...
_ALLOWED_REPORT_DIRS = [
    pathlib.Path(d).resolve()
    for d in ["./reports", os.getenv("REPORT_OUTPUT_DIR", "./reports/output")]
]
...
@router.get(
    "/{report_id}/download", response_class=FileResponse, summary="Download incident report PDF"
)
...
async def report_stats(session: AsyncSession = Depends(get_session)) -> ReportSummary:
    ...
    sev_rows = (
        await session.execute(
            text("SELECT severity_level, COUNT(*) FROM incident_reports GROUP BY severity_level")
        )
    ).all()
    sev = {r[0]: r[1] for r in sev_rows if r[0]}
    cls_rows = (
        await session.execute(
            text("SELECT class_name, COUNT(*) FROM incident_reports GROUP BY class_name")
        )
    ).all()
    ...
    avg = float(
        (
            await session.execute(
                text(
                    "SELECT AVG(generation_ms) FROM incident_reports WHERE generation_ms IS NOT NULL"
                )
            )
        ).scalar()
        or 0
    ...
    return ReportSummary(
        total_reports=total, by_severity=sev, by_class=cls, avg_generation_ms=round(avg, 1)
    )
