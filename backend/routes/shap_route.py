...
from ..models import ErrorResponse, SHAPResponse
...
async def get_shap_explanation(
    ...
    if not success:
        ...
        raise HTTPException(
            status.HTTP_500_INTERNAL_SERVER_ERROR, "Failed to encode saliency overlay"
        )
...
def _run_shap(explainer: Any, frame_bgr: np.ndarray, detection: Any) -> tuple:
    ...
    if crop.size == 0:
        raise ValueError(
            f"Empty crop region for bbox ({x1},{y1},{x2},{y2}) " f"with pad={pad}, frame={fw}x{fh}"
        )
