...
class OrgUpdate(BaseModel):
    ...
    max_cameras: int | None = Field(default=None, ge=1, le=500)
    max_sites: int | None = Field(default=None, ge=1, le=100)
    max_users: int | None = Field(default=None, ge=1, le=1000)


def _generate_org_id(name: str) -> str:
    """Generate a unique org_id from name."""
    slug = "".join(c.lower() if c.isalnum() else "-" for c in name[:20]).strip("-")
...
async def get_org_usage(
    ...
    # Count usage
    cam_count = (
        await session.exec(text("SELECT COUNT(*) FROM camera_registry WHERE 1=1"))
    ).scalar() or 0

    site_count = (
        await session.exec(text("SELECT COUNT(*) FROM sites WHERE active = 1"))
    ).scalar() or 0
