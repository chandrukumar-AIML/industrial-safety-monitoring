...
def build_safety_agent(
    db_factory: DBFactoryProtocol | None = None,
    ...
    graph.add_node(
        "check_worker_history",
        _inject_deps(node_check_worker_history, db_factory=db_factory)
        if db_factory
        else node_check_worker_history,
    )
    graph.add_node(
        "score_severity",
        _inject_deps(node_score_severity, llm_client=llm_client)
        if llm_client
        else node_score_severity,
    )
    ...
    graph.add_node(
        "log_to_database",
        _inject_deps(node_log_to_database, db_factory=db_factory)
        if db_factory
        else node_log_to_database,
    )
    graph.add_node(
        "update_compliance",
        _inject_deps(node_update_compliance, db_factory=db_factory)
        if db_factory
        else node_update_compliance,
    )
...
_safety_agent_instance: CompiledStateGraph | None = None
