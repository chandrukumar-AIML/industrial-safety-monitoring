...
async def get_zone_info(
    ...
    if not row:
        ...
        return ZoneInfo(zone_id=zone_id, zone_name="Unknown", zone_type="unknown", required_ppe=[])
...
async def update_compliance_score(
    ...
    new_score = (
        float(row["score"]) if row else max(COMPLIANCE_MIN, min(COMPLIANCE_MAX, 100.0 + delta))
    )
...
async def log_agent_run(
    ...
    if state.get("redact_pii", False):
        trace_steps = [
            {**step, "details": {k: _redact(v, k) for k, v in step.get("details", {}).items()}}
