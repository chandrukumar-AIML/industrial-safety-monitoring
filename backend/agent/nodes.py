...
def _redact_trace(data: dict, redact: bool = REDACT_PII) -> dict:
    ...
    if not redact:
        return data
    return {k: ("***" if k in {"track_id", "worker_id", "zone_id"} else v) for k, v in data.items()}
...
async def node_detect_violation(state: AgentState) -> dict[str, Any]:
    ...
    if not isinstance(event["track_id"], int) or event["track_id"] < 0:
        return {
            ...
            "trace_steps": _trace(
                state, node_name, {"status": "SKIPPED", "reason": "Invalid track_id"}
            ),
        }
    ...
    if not isinstance(event["confidence"], int | float) or not 0 <= event["confidence"] <= 1:
        return {
            ...
            "trace_steps": _trace(
                state, node_name, {"status": "SKIPPED", "reason": "Invalid confidence"}
            ),
        }
...
async def node_score_severity(state: AgentState) -> dict[str, Any]:
    ...
    try:
        ...
        score, reason = (
            rule_score,
            f"Rule-based: {class_name_raw} in {event.get('zone_id', 'unknown')}",
        )
