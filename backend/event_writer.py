...
ZONE_RISK_SNAPSHOT_INTERVAL: int = int(__import__("os").getenv("ZONE_RISK_SNAPSHOT_INTERVAL", "30"))
...
MAX_BATCH_SIZE: int = int(__import__("os").getenv("EVENT_WRITER_MAX_BATCH_SIZE", "100"))
...
async def _persist_violations(
    violations: list[TrackedDetection],  # type: ignore[name-defined]  # noqa: F821
    ...
    async with session_factory() as session:
        try:
            for viol in batch:
                ...
                if x1 >= x2 or y1 >= y2:
                    logger.warning(
                        "Skipping violation with invalid bbox order: {}",
                        bbox,
                    )
                    continue
    ...
    async with session_factory() as session:
        try:
            for zr in zone_risks:
                ...
                if not zone_id:
                    logger.warning("Skipping zone risk entry missing 'zone_id': {}", zr)
                    continue
                ...
                if (
                    not 0 <= mean_intensity <= 1
                    or not 0 <= max_intensity <= 1
                    or not 0 <= violation_pct <= 1
                ):
                    logger.warning("Skipping zone risk with invalid intensity/pct values: {}", zr)
                    continue
...
async def start_event_writer(
    ...
    try:
        async for frame_result in pipeline.results():
            ...
            if frame_count % ZONE_RISK_SNAPSHOT_INTERVAL == 0 and frame_result.zone_risks:
