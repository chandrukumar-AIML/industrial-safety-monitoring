...
CHUNK_SIZE = _validate_int_range(
    "RAG_CHUNK_SIZE", os.getenv("RAG_CHUNK_SIZE", "512"), 512, 100, 2000
)
CHUNK_OVERLAP = _validate_int_range(
    "RAG_CHUNK_OVERLAP", os.getenv("RAG_CHUNK_OVERLAP", "64"), 64, 0, 500
)

# Security: restrict file paths
ALLOWED_INGEST_DIRS = [
    os.path.abspath(d.strip())
    for d in os.getenv("ALLOWED_INGEST_DIRS", "./data").split(",")
    if d.strip()
]
...
class DBSessionProtocol(Protocol):
    ...
    async def execute(self, query, params: dict | None = None): ...
...
class SafetyDocumentIngestor:
    ...
    def __init__(
        ...
        chroma_client: Any | None = None,  # Injected for testing
        embeddings: Any | None = None,  # Injected for testing
    ):
        cfg = config or IngestConfig()

        # Validate and set paths
        self._chroma_db_dir = _validate_path(
            cfg.chroma_db_dir, ALLOWED_INGEST_DIRS, "chroma_db_dir"
        )
        self._safety_docs_dir = _validate_path(
            cfg.safety_docs_dir, ALLOWED_INGEST_DIRS, "safety_docs_dir"
        )
    ...
    async def ingest_violations_db(
        ...
        for (day, zone), events in groups.items():
            ...
            violation_summary = ", ".join(f"{count} '{cls}'" for cls, count in class_counts.items())
...
_ingestor_instance: SafetyDocumentIngestor | None = None
