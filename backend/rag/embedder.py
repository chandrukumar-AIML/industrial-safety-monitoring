...
if not 1 <= EMBEDDING_BATCH_SIZE <= 256:
    logger.warning("EMBEDDING_BATCH_SIZE out of range: {} — using 32", EMBEDDING_BATCH_SIZE)
...
class HuggingFaceEmbedder:
    ...
    def __init__(
        ...
        embedding_impl: object | None = None,
    ):
        """
    ...
    def _load_embedding_model(self) -> object:
        ...
        try:
            ...
            logger.error("Failed to load embedding model '{}': {}", self._model_name, e)
            raise EmbeddingError(
...
_embedder_instance: HuggingFaceEmbedder | None = None
...
def get_embedder(
    ...
    **kwargs,
    ...
    if _embedder_instance is None:
        _embedder_instance = HuggingFaceEmbedder(model_name=model_name, device=device, **kwargs)
